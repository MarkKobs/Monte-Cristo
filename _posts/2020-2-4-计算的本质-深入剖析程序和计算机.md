...
我们有时需要在文件中定义好一些类或模块以便重用，irb中可以加`-r`参数加载。注意如果要使用到文件中定义的变量，一定得是全局变量，以大写字母开头，若变量以小写字母开头，为局部变量。
...
什么是计算？实现一个计算，需要三个基本要素：

- 一台**机器**，能够执行计算
- 一种**语言**，用来编写这台机器可以理解的指令
- 一个**程序**，用这种语言编写，描述机器应该具体执行哪些计算

在理解了一种语言的含义后，就可以把一段没有生命的源代码和一个动态的进程联系起来。
...
下面的程序仅有加法、乘法、数字三个类型，以及一个虚拟机，它能记录当前的状态，即表达式。
...
在一门语言中，我们不光使用字面值，用有意义的变量名指代非常必要，所以引入Varible来表示变量。
